    ...
    def _packet_in_handler(self, ev):
        ...
        if tcp_header is not None:
            ...
            if tcp_flags & tcp.TCP_SYN:
                print(111)
            if tcp_flags & tcp.TCP_FIN:
                print(222)
        # if tcp_header is not None:
        #     tcp_match =  parser.OFPMatch(in_port=in_port, eth_type=0x0800, ip_proto=6, eth_dst=dst, eth_src=src)
        #     tcp_actions = [parser.OFPActionSetRWND(4),parser.OFPActionOutput(out_port)]
        #     self.add_flow(datapath, 10, tcp_match, tcp_actions)
